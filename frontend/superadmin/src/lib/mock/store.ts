...
import type { Branch, BranchInput } from "@/features/branches/types";
import type {
  Template,
  TemplateAssignment,
...
let templates: Template[] = mockTemplatesSeed.map((t) => ({
  ...t,
  assignments: t.assignments.map((a) => ({ ...a })),
  images: [...t.images],
...
let branchesList: Branch[] = mockBranchesSeed.map((b) => ({ active: true, ...b }));
...
export function mockCreateBranch(input: BranchInput): Branch {
  const branch: Branch = { _id: newId("branch"), ...input };
  branchesList = [branch, ...branchesList];
  return branch;
}

export function mockUpdateBranchRecord(id: string, input: BranchInput): Branch {
  const existing = branchesList.find((b) => b._id === id);
  if (!existing) throw new Error("Branch not found");
  Object.assign(existing, input);
  return existing;
}

export function mockDeleteBranchRecord(id: string): void {
  branchesList = branchesList.filter((b) => b._id !== id);
}
...
export function mockCreateService(input: ServiceInput): Service {
  const service: Service = { _id: newId("service"), approvalStatus: "PENDING", ...input };
...
export function mockCreateOffer(input: OfferInput): Offer {
  const offer: Offer = { _id: newId("offer"), approvalStatus: "PENDING", ...input };
  offers = [offer, ...offers];
  return offer;
}
...
// A template's branch list is derived from its assignment records, not a
// flat array — filtering by branch shows only templates assigned there.

export function mockFetchTemplateList(branchId?: string): Template[] {
  if (!branchId) return templates;
  return templates.filter((t) => t.assignments.some((a) => a.branchId === branchId));
}
...
// Assigning a branch that already has an assignment on this template updates
// that record (date/status) in place, rather than duplicating it — same
// pattern as product branch allocations.
export function mockAssignTemplateToBranch(id: string, input: TemplateAssignmentInput): Template {
  const existing = templates.find((t) => t._id === id);
  if (!existing) throw new Error("Template not found");
  const assignment = existing.assignments.find((a) => a.branchId === input.branchId);
  if (assignment) {
    assignment.assignedDate = input.assignedDate;
    assignment.status = input.status;
  } else {
    const record: TemplateAssignment = { _id: newId("tassign"), ...input };
    existing.assignments = [...existing.assignments, record];
  }
  return existing;
}

export function mockRemoveTemplateAssignment(id: string, assignmentId: string): Template {
  const existing = templates.find((t) => t._id === id);
  if (!existing) throw new Error("Template not found");
  existing.assignments = existing.assignments.filter((a) => a._id !== assignmentId);
  return existing;
}
...
export function mockCreateProduct(input: ProductInput): Product {
  const product: Product = { _id: newId("product"), allocations: [], approvalStatus: "PENDING", ...input };
