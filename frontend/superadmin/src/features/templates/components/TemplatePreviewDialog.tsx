...
export function TemplatePreviewDialog({
  ...
  const heroImage = hasBeforeAfter
    ...
    : template.imageUrl;
  const assignedBranches = template.assignments
    .map((a) => {
      const name = branches.find((b) => b._id === a.branchId)?.name;
      return name ? { name, status: a.status, assignedDate: a.assignedDate } : undefined;
    })
    .filter(
      (v): v is { name: string; status: "assigned" | "pending"; assignedDate: string } => !!v
    );
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-md border p-2.5 text-center">
              ...
              <div className="text-sm font-medium">{template.assignments.length}</div>
            </div>
            <div className="rounded-md border p-2.5 text-center">
                ...
                {assignedBranches.map((b) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 pl-[18px] text-muted-foreground">
                        {safeDate(b.assignedDate)}
                      </div>
                    </div>
