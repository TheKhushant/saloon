...
export default function TemplatesPage() {
  ...
  const analytics = useMemo(() => {
    ...
    const mostUsed = [...list].sort((a, b) => b.assignments.length - a.assignments.length)[0];
  ...
  const assignMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: TemplateAssignmentInput }) =>
      assignTemplateToBranch(id, input),
    onSuccess: () => {
      toast.success("Template assigned to branch");
      setAssignOpen(false);
      invalidate();
    },
    onError: (e: unknown) => toast.error(extractMessage(e, "Failed to assign template")),
  });

  const unassignMutation = useMutation({
    mutationFn: ({ id, assignmentId }: { id: string; assignmentId: string }) =>
      removeTemplateAssignment(id, assignmentId),
    onSuccess: () => {
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {filtered.map((t, i) => (
            <TemplateCard
              ...
              onUnassign={(assignmentId) => unassignMutation.mutate({ id: t._id, assignmentId })}
            />
          ))}
        </div>
      )}
