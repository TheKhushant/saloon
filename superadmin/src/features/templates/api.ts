...
export async function assignTemplateBranches(id: string, branchIds: string[]): Promise<Template> {
  return withMockFallback(
    async () => {
      const { data } = await api.patch<Template>(`/admin/templates/${id}/branches`, { branchIds });
      return data;
    },
    () => mockAssignTemplateBranches(id, branchIds)
  );
}
